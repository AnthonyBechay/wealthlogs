...
class EmailService {
  constructor() {
    ...
    if (!this.fromEmail) {
      console.warn('⚠️ FROM_EMAIL is not set - using default sender address');
    }
  }
  ...
  async sendEmail(to, subject, html, text) {
    ...
    if (process.env.NODE_ENV === 'production' && 
      ...
      console.warn(`Blocked email to test domain in production: ${to}`);
    ...
    try {
      ...
      return result; // Retourner l'objet complet plutôt que juste true
    } catch (error) {
  ...
  async sendVerificationEmail(userEmail, username, verificationToken) {
    ...
    try {
      ...
      // En développement, ne pas faire échouer l'inscription si l'email échoue
  ...
  async sendWelcomeEmail(userEmail, username) {
    ...
    try {
      ...
      // L'email de bienvenue est moins critique, on peut continuer sans faire échouer
      if (process.env.NODE_ENV === 'development') {
        console.warn('Welcome email failed in development, continuing anyway...');
        return { warning: 'Welcome email could not be sent but verification succeeded' };
      }
      
      // En production, on peut aussi choisir de ne pas faire échouer
      console.warn('Welcome email failed but verification succeeded');
      return { warning: 'Welcome email could not be sent but verification succeeded' };
    }
  }
