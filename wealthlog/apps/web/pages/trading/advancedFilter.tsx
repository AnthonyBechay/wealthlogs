...
interface FXRow {
  id: number; instrument: string; entryDate: string; tradeDirection: "LONG"|"SHORT";
  lots: number|null; entryPrice:number|null; exitPrice:number|null;
  stopLossPips:number|null; pipsGain:number|null; amountGain:number|null; percentageGain:number|null; fees:number;
}
...
export default function AdvFilter() {
  ...
  useEffect(()=>{ (async()=>{
    ...
    const s = await api.get("/settings"); setInstList(s.data.instruments||[]);
  })(); },[]);
  ...
  const [minLots,setMinLots]     = useState(""); const [maxLots,setMaxLots]="";
  const [minPct,setMinPct]       = useState(""); const [maxPct,setMaxPct]="";
  const [groupBy,setGroupBy]     = useState<"instrument"|"direction"|"month">("instrument");

  /*── query runner ─*/
  async function run(e?:React.FormEvent){ e?.preventDefault(); setLoad(true); setErr("");
    try{
      const qs = new URLSearchParams({
        accountId,instr,symbol:instr,dir,from,to,
        pattern,minLots,maxLots,minPct,maxPct,groupBy
      });
  ...
  /*── export helpers ─*/
  const exportCsv = ()=>{ const csv=Papa.unparse(rows); const blob=new Blob([csv],{type:"text/csv"});
    const url=URL.createObjectURL(blob); const a=document.createElement("a");
    a.href=url; a.download="trades.csv"; a.click(); URL.revokeObjectURL(url); };
  const exportJson = ()=>{ const blob=new Blob([JSON.stringify({rows,bks},null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob); const a=document.createElement("a");
  ...
  <div className="min-h-screen p-4 bg-[var(--background)] text-[var(--text)]">
    <div className="flex items-center justify-between mb-4">
      ...
      <button onClick={()=>router.push("/trading")} className="text-sm underline">← back</button>
    </div>
...
className="bg-[var(--background-2)] p-4 rounded-lg grid gap-3 grid-cols-1 md:grid-cols-3"
 >
      ...
      <select className="input" value={dir} onChange={e=>setDir(e.target.value as any)}>
        <option value="">Direction</option><option>LONG</option><option>SHORT</option>
      </select>
      ...
      <select className="input" value={pattern} onChange={e=>setPattern(e.target.value)}>
        <option value="">All patterns</option><option value="Breakout">Breakout</option>
        <option value="Reversal">Reversal</option><option value="Range">Range</option>
      </select>
      ...
      <div className="flex gap-2 col-span-full">
        <button type="button" onClick={()=>{setFrom(startOfMonthISO());setTo(todayISO());}}
          className="btn-sm">This month</button>
        <button type="button" onClick={()=>{setFrom("");setTo("");}}
          className="btn-sm">Any date</button>
        <button type="button" onClick={()=>setMoreOpen(v=>!v)} className="btn-sm">
          {moreOpen? "− Less": "+ More"}
        </button>
      ...
      {moreOpen && (
        <>
          ...
          <input className="input" placeholder="Min % gain" value={minPct} onChange={e=>setMinPct(e.target.value)}/>
          <input className="input" placeholder="Max % gain" value={maxPct} onChange={e=>setMaxPct(e.target.value)}/>
        </>
      )}
    </form>
      ...
      <>
        ...
        <div className="overflow-x-auto mb-6">
          <table className="w-full border-collapse text-sm">
            <thead><tr>
              {["Key","Trades","Gross $","Avg %","Win %"].map(h=><th key={h} className="th">{h}</th>)}
            </tr></thead>
            <tbody>
              {bks.map(b=>(
                <tr key={b.key}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-xs md:text-sm">
            <thead>
              <tr>{["Date","Sym","Dir","Lots","$","%","Entry","Exit","SL","Pips"].map(h=><th key={h} className="th">{h}</th>)}</tr>
            </thead>
            <tbody>
              {rows.map(r=>(
                <tr key={r.id}>
                  ...
                  <td className="td">{r.percentageGain!=null?(r.percentageGain*100).toFixed(2):"-"}</td>
    ...
    <style jsx>{`
      .input{padding:.45rem;border-radius:.25rem;background:var(--background);border:1px solid var(--border)}
      .btn{padding:.5rem 1rem;background:var(--primary);color:#fff;border-radius:.25rem}
      .btn-sm{padding:.35rem .75rem;background:var(--background-2);border:1px solid var(--border);border-radius:.25rem;text-sm}
      .btn.primary{background:var(--primary);color:#fff}
      .th{border-bottom:1px solid var(--border);padding:.4rem;text-align:left;white-space:nowrap}
      .td{border-bottom:1px solid var(--border);padding:.4rem;white-space:nowrap}
    `}</style>
  </div>);
}
