...
export default function Settings() {
  ...
  async function handleAddInstrument() {
    ...
    try {
      const res = await api.post("/settings/instruments/add", { instrument: newInstrument });
      setInstruments(res.data.instruments);
      setNewInstrument("");
    } catch (err) {
  ...
  async function handleDeleteInstrument(instrument: string) {
    try {
      const res = await api.post("/settings/instruments/delete", { instrument });
      setInstruments(res.data.instruments);
    } catch (err) {
  ...
  async function handleUpdateInstrument(oldValue: string) {
    ...
    try {
      const res = await api.post("/settings/instruments/edit", {
  ...
  async function handleAddPattern() {
    ...
    try {
      const res = await api.post("/settings/patterns/add", { pattern: newPattern });
      setPatterns(res.data.patterns);
      setNewPattern("");
    } catch (err) {
  ...
  async function handleDeletePattern(pattern: string) {
    try {
      const res = await api.post("/settings/patterns/delete", { pattern });
      setPatterns(res.data.patterns);
    } catch (err) {
  ...
  async function handleUpdatePattern(oldValue: string) {
    ...
    try {
      const res = await api.post("/settings/patterns/edit", {
        oldPattern: oldValue,
  ...
  async function updateBeRange() {
    ...
    try {
      await api.post("/settings/beRange/update", { beMin: minVal, beMax: maxVal });
      fetchSettings();
    } catch (err) {
  ...
  async function createCommunity() {
    ...
    try {
      await api.post("/community/create", { name: communityName });
      setCommunityName("");
      fetchAllCommunities();
      fetchUserCommunities();
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "#FAFAFA", color: "#37474F" }}>
              ...
              {allAccountsForDelegation.map((acct) => (
                <label key={acct.id} className="flex items-center gap-1">
                  <input
                    ...
                    onChange={() => toggleCoachAccount(acct.id)}
                  />
                  {acct.name}
                </label>
              ))}
