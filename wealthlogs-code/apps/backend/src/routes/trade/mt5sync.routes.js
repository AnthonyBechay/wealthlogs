...
const normInstrument = sym => sym.toUpperCase().replace(/[^A-Z0-9]/g, '');
...
router.post('/', async (req, res) => {
  ...
  /* sanitise fee so it’s always positive */
  ...
  try {
    ...
    if (dirNorm && dirNorm !== 'buy' && dirNorm !== 'sell')
      return res.status(400).json({ error: 'Invalid direction value (use Buy or Sell)' });

    const tradeDirection =
      dirNorm     ? (dirNorm === 'buy' ? 'LONG' : 'SHORT')
                  : (volume >= 0       ? 'LONG' : 'SHORT');   // fallback
